/** @doc System status — pure black background with status cards. */
    ...
    <div className="min-h-screen w-full bg-black text-[#f5f5f5]">
      <div className="mx-auto w-full max-w-md px-6 py-8">
        ...
        <div className="flex flex-col items-center pb-4 pt-2 text-center">
          ...
          <h2
            className="mt-5 text-[22px] leading-tight font-semibold tracking-tight text-[#f5f5f5]"
            style={{ fontFamily: '"Space Grotesk", "Inter", system-ui, sans-serif' }}
          >
            {loading
              ? "Checking systems…"
            ...
            return (
              <div key={svc.name}>
                <div className="mb-2 flex items-center justify-between px-0.5">
                  <span className="text-[14px] text-[#f5f5f5]">{svc.name}</span>
                  <span className="text-[12px] tabular-nums text-[#a0a0a0]">
                    {uptime.toFixed(2)}%
                  </span>
                </div>
        ...
        <div className="mt-12">
          <h3
            className="mb-4 px-0.5 text-[15px] font-semibold tracking-tight text-[#f5f5f5]"
            style={{ fontFamily: '"Space Grotesk", "Inter", system-ui, sans-serif' }}
          >
            Past incidents
          </h3>
          {incidents.length === 0 ? (
            <div className="rounded-2xl border border-dashed border-[#1a1a1a] py-10 text-center">
              <p className="text-[13.5px] text-[#a0a0a0]">Nothing here yet.</p>
            </div>
          ) : (
            <div className="divide-y divide-[#1a1a1a] border-b border-t border-[#1a1a1a]">
                  ...
                  <div key={incident.id} className="flex items-start justify-between gap-3 py-4">
                    <div className="flex min-w-0 items-start gap-2.5">
                      ...
                      <div className="min-w-0">
                        <div className="truncate text-[14px] text-[#f5f5f5]">
                          {incident.title || `${incident.service_name} is down`}
                        </div>
                        <div className="mt-0.5 text-[12px] text-[#a0a0a0]">
                          {incident.message || "Service disruption"}
                        </div>
                      </div>
                    </div>
                    <div className="shrink-0 text-right">
                      <div className="text-[12px] tabular-nums text-[#a0a0a0]">
