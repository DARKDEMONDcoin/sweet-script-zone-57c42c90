...
export function useChatAuiRuntime(params: {
  ...
  const feedbackAdapter = useMemo<FeedbackAdapter | undefined>(() => {
    if (!onFeedback) return undefined;
    return {
      submit: ({ message, type }) => {
        ...
        if (idx < 0) return;
        onFeedback(idx, type === "positive" ? true : type === "negative" ? false : null);
      },
    };
  }, [onFeedback, messages]);
  ...
  return useExternalStoreRuntime({
    ...
    onEdit: onEdit
      ? async (message: AppendMessage) => {
          const text = message.content
            ...
            .map((c) => c.text)
            .join("");
          // parentId is the message being edited (the user message)
    ...
    onReload: onReload
      ...
      : undefined,
    onCancel: onCancel ? async () => { await onCancel(); } : undefined,
  });
}
