...
export function ChatModelSettingsPanel() {
  ...
  return (
    <div className="space-y-3">
      <div className="flex flex-col rounded-2xl border border-border/50 bg-muted/40 overflow-hidden">
        {OPTIONS.map(({ id, icon: Icon, description }) => {
          ...
          return (
            <button
              ...
              className={`flex w-full items-center gap-3 px-4 py-3.5 text-start transition-colors border-b border-border/50 last:border-b-0 ${active ? "bg-muted/40" : "hover:bg-muted/40"}`}
            >
              <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-muted/40"><Icon className="h-4 w-4 text-foreground/80" /></span>
