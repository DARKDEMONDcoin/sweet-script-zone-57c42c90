...
import { MessageByIndexProvider } from "@assistant-ui/react";
  ...
  function ChatMessagesList(props, messagesEndRef) {
    ...
    return (
      ...
      >
        ...
        {(() => {
          // Map raw index → visible index so MessageByIndexProvider aligns with
          // the runtime's filtered messages (which drops hiddenFromTranscript).
          let visibleCursor = -1;
          return messages.map((msg, i) => {
            if (msg.hiddenFromTranscript) return null;
            visibleCursor += 1;
            const visibleIndex = visibleCursor;
            return (
              <div
              ...
              >
                <MessageByIndexProvider index={visibleIndex}>
                  <ChatMessageItem
                    msg={msg}
                    i={i}
                    messages={messages}
